...
DocumentHandler.prototype.handleGet = function(key, response, skipExpire) {
  this.store.get(key, function(ret) {
    if (ret) {
      logger.verbose('retrieved document', { key: key });
      response.writeHead(200, { 'content-type': 'application/json' });
      response.end(JSON.stringify({ data: ret, key: key }));
    }
    else {
      logger.warn('document not found', { key: key });
...
DocumentHandler.prototype.handleRawGet = function(key, response, skipExpire) {
  this.store.get(key, function(ret) {
    if (ret) {
      logger.verbose('retrieved raw document', { key: key });
      response.writeHead(200, { 'content-type': 'text/plain; charset=UTF-8' });
      response.end(ret);
    }
    else {
      logger.warn('raw document not found', { key: key });
      response.writeHead(404, { 'content-type': 'application/json' });
      response.end(JSON.stringify({ message: 'Document not found.' }));
    }
  }, skipExpire);
...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var onSuccess = function () {
    ...
    if (_this.maxLength && buffer.length > _this.maxLength) {
      cancelled = true;
      logger.warn('document >maxLength', { maxLength: _this.maxLength });
    ...
    _this.chooseKey(function (key) {
      _this.store.set(key, buffer, function (res) {
        if (res) {
          logger.verbose('added document', { key: key });
          response.writeHead(200, { 'content-type': 'application/json' });
